...
OUTPUT="${JUICE_X86_SMOKE_OUTPUT:-$ROOT/build/x86-smoke.exe}"

case "$OUTPUT" in
  "$ROOT"/build/*) ;;
  *) test "${JUICE_ALLOW_EXTERNAL_BUILD:-0}" = 1 || {
       echo "Refusing i386 smoke output outside build/: $OUTPUT" >&2
       exit 2
     };;
esac
...
"$TOOLCHAIN/bin/i686-w64-mingw32-clang" \
  -Os -fno-builtin -fno-stack-protector -nostdlib \
  -Wl,--entry,mainCRTStartup -Wl,--subsystem,windows \
  "$ROOT/tests/x86/smoke.c" -lkernel32 -o "$OUTPUT"

machine="$("$TOOLCHAIN/bin/llvm-readobj" --file-headers "$OUTPUT" |
  sed -n 's/^  Machine: //p')"
case "$machine" in
  IMAGE_FILE_MACHINE_I386*) ;;
  *) echo "Unexpected i386 smoke-test machine: $machine" >&2; exit 3;;
esac
sha256sum "$OUTPUT" > "$OUTPUT.sha256"
echo "JUICE_X86_SMOKE_BUILD_OK path=$OUTPUT machine=$machine"
