...
That revision is recorded in config/wine-base.txt. The Juice delta modifies 28
upstream paths and adds the six-file dlls/wineios.drv driver plus one provenance
file, for 35 patch paths in total. Both configure.ac and the generated configure
file include the new driver so a release checkout does not require Autoconf
just to register it.
...
the current patch exactly describes the modified content it touches, including
all six new driver files.
